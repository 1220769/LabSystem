...
import { downloadResultadoPDF, downloadRelatorioPDF } from '../utils/relatorioPdf'
import './Portal.css'
...
/* ── PDF (download real para as Transferências) ── */
function printRelatorioSimples(r: IRes) {
  downloadResultadoPDF({
    ...
    subtitulo:       'Resultado Clínico',
    meta:            `${r.requisicaoNumero} · ${fmtDate(r.createdAt)}`,
    validacoes:      r.validacaoMedica
      ? [{ label: 'Validado por', nome: r.validacaoMedica.nome, quando: fmtDateTime(r.validacaoMedica.dataHora) }]
      : undefined,
  })
}
...
async function printRelatorioCompleto(reqNumero: string, utenteNome: string, setMsg: (m: string) => void) {
  ...
  try {
    ...
    downloadRelatorioPDF({
      subtitulo:  'Relatório de Análises Clínicas',
      utenteNome,
      metaLinha:  `${reqNumero} · ${fmtDate(resultados[0]?.createdAt)}`,
      colunas:    ['Análise', 'Resultado', 'Referência', 'Flag'],
      linhas: resultados.map(res => [
        res.analise.nome,
        `${res.valor ?? '—'} ${res.unidade ?? ''}`.trim(),
        res.refMin !== undefined ? `${res.refMin} – ${res.refMax}` : '—',
        res.flag.replace(/_/g, ' '),
      ]),
      ficheiro: `Relatorio_${reqNumero}.pdf`,
    })
  ...
  const handlePedido = async () => {
    ...
    try {
      ...
      await api.post('/requisicoes', {
        ...
        utenteProcesso:   (perfil as any)?.numeroProcesso ?? '',
  ...
  if (noLink) return (
    <div className="portal">
      <header className="portal-hd">
        <div className="portal-logo">Lab<strong>System</strong></div>
