...
import { downloadResultadoPDF, downloadRelatorioPDF, FLAG_HEX } from '../utils/relatorioPdf'
...
export default function PortalMedico() {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      setStats({
        // nº de requisições prontas para validação médica (igual à lista) —
        // evita mostrar um contador que não corresponde a nada na lista
        aguardamMedica:    rs.data.reqsValidadoTecnico ?? 0,
  ...
  useEffect(() => {
    ...
    valTimer.current = setTimeout(() => { setValDeb(valSearch) }, 300)
  ...
  /* ── relatório de UM resultado → download PDF (Transferências) ── */
  const printRelatorio = (r: IResultado) => {
    const validacoes: { label: string; nome: string; quando: string; obs?: string }[] = []
    if (r.validacaoTecnica) validacoes.push({ label: 'Validação Técnica', nome: r.validacaoTecnica.nome, quando: fmtH(r.validacaoTecnica.dataHora) })
    if (r.validacaoMedica)  validacoes.push({ label: 'Validação Médica',  nome: r.validacaoMedica.nome,  quando: fmtH(r.validacaoMedica.dataHora), obs: r.validacaoMedica.observacoes })
    downloadResultadoPDF({
      codigoResultado: r.codigoResultado,
      subtitulo: 'Resultado Clínico · Validado Medicamente',
      utenteNome: r.utenteNome,
      meta: `${r.requisicaoNumero} · ${r.codigoAmostra} · ${fmt(r.createdAt)}`,
      analiseNome: r.analise.nome,
      analiseCodigo: r.analise.codigo,
      valor: r.valor,
      unidade: r.unidade,
      flag: r.flag,
      flagLabel: FLAG_LABEL[r.flag],
      refMin: r.refMin,
      refMax: r.refMax,
  ...
  /* ── relatório COMPLETO da requisição → download PDF (Transferências) ── */
  const printReqPDF = (reqNum: string, items: IResultado[]) => {
    ...
    downloadRelatorioPDF({
      subtitulo: 'Relatório de Resultados · Validado Medicamente',
      utenteNome: utente,
      metaLinha: `${reqNum} · ${amostra} · ${date} · Validado por ${user?.nome ?? '—'}`,
      colunas: ['Análise', 'Categoria', 'Resultado', 'Referência', 'Flag', 'Observações'],
      linhas: items.map(r => [
        r.analise.nome,
        r.analise.categoria,
        `${r.valor ?? '—'} ${r.unidade ?? ''}`.trim(),
        r.refMin !== undefined && r.refMax !== undefined ? `${r.refMin} – ${r.refMax} ${r.unidade ?? ''}`.trim() : '—',
        FLAG_LABEL[r.flag],
        r.observacoes ?? '—',
      ]),
      corFlagPorLinha: items.map(r => FLAG_HEX[r.flag] ?? '#1A1208'),
      ficheiro: `Relatorio_${reqNum}.pdf`,
    })
  }
  ...
  return (
    <div className="pm-page">
      ...
      <header className="pm-hd">
        <div className="pm-logo">Lab<strong>System</strong></div>
