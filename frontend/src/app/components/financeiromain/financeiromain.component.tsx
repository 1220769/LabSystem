...
export default function FinanceiromainComponent() {
  ...
  /* ─── exportar XML (validado pelo fatura.xsd no servidor) ─── */
  const exportarXML = async () => {
    setXmlBusy(true); setXmlResult(null)
    try {
      const r = await api.get('/faturas/export/xml', { responseType: 'text' })
      const blob = new Blob([r.data], { type: 'application/xml;charset=utf-8' })
      const url  = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = url
      a.download = `faturas-${new Date().toISOString().slice(0,10)}.xml`; a.click()
      URL.revokeObjectURL(url)
    } catch {
  ...
  /* ─── importar XML (validado contra o XSD antes de gravar) ─── */
  const importarXML = async (file: File) => {
    setXmlBusy(true); setXmlResult(null)
    try {
      const text = await file.text()
      const r = await api.post('/faturas/import/xml', text, { headers: { 'Content-Type': 'application/xml' } })
      const d  = r.data
      const extra = d.ignoradas > 0 ? ` · ${d.ignoradas} ignorada${d.ignoradas !== 1 ? 's' : ''} (nº inexistente)` : ''
      setXmlResult({ ok: true, msg: `${d.message} ${d.atualizadas} fatura${d.atualizadas !== 1 ? 's' : ''} atualizada${d.atualizadas !== 1 ? 's' : ''}${extra}.` })
      setTab('faturas')
      ...
      <section className="staff-hero">
        <div>
          ...
          <div className="staff-actions">
            ...
            <button className="staff-btn-ghost"   onClick={exportarXML} disabled={xmlBusy}>↧ exportar XML</button>
            <button className="staff-btn-ghost"   onClick={() => fileRef.current?.click()} disabled={xmlBusy}>↥ importar XML</button>
            <input ref={fileRef} type="file" accept=".xml,application/xml,text/xml" style={{ display: 'none' }}
      ...
      <main className="staff-content">
        ...
        {tab === 'faturas' && (
          <div className="fin-section">
            {xmlResult && (
              <div className={`fin-xml-banner fin-msg ${xmlResult.ok ? 'fin-msg--ok' : 'fin-msg--err'}`}>
                <div className="fin-xml-banner-head">
                  <span>{xmlResult.ok ? '✓' : '✕'} {xmlResult.msg}</span>
                  <button className="fin-xml-close" onClick={() => setXmlResult(null)}>×</button>
                </div>
                {xmlResult.errors && xmlResult.errors.length > 0 && (
                  <ul className="fin-xml-errs">
                    {xmlResult.errors.map((er, i) => (
                      <li key={i} className="fin-xml-err-line">
                        {er.line != null && <span className="fin-xml-err-no">linha {er.line}</span>}
                        {er.message}
                      </li>
                    ))}
                  </ul>
