...
interface IStats {
  ...
  // contagens por REQUISIÇÃO (não por análise) — alinham com as listas agrupadas
  reqsPendentes?: number; reqsDisponiveis?: number
}
...
/* só permite dígitos e um único separador decimal (aceita vírgula → ponto).
   Bloqueia letras, espaços e símbolos: o técnico não consegue meter letras. */
...
export default function TecnicomainComponent() {
  ...
  const [xmlResult, setXmlResult] = useState<{ ok: boolean; msg: string; errors?: { message: string; line?: number }[] } | null>(null)
  ...
  const setReqField = (id: string, k: keyof IForm, v: string) => {
    // campos numéricos: aceitam apenas números (sem letras)
    const val = (k === 'valor' || k === 'refMin' || k === 'refMax') ? soNumero(v) : v
    setReqForms(prev => {
      const f = { ...prev[id], [k]: val }
  ...
  /* ─── intercâmbio XML/XSD (export · validação · import) ─── */
  const exportXML = async (kind: 'requisicoes' | 'resultados') => {
    setXmlBusy(true); setXmlResult(null)
    try {
      const r = await api.get(`/${kind}/export/xml`, { responseType: 'text' })
      const blob = new Blob([r.data], { type: 'application/xml;charset=utf-8' })
      const url  = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = url
      a.download = `${kind}-${new Date().toISOString().slice(0, 10)}.xml`; a.click()
      URL.revokeObjectURL(url)
    } catch {
      setXmlResult({ ok: false, msg: 'Erro ao exportar XML.' })
    } finally { setXmlBusy(false) }
  }

  const importXML = async (kind: 'requisicoes' | 'resultados', file: File) => {
    setXmlBusy(true); setXmlResult(null)
    try {
      const txt = await file.text()
      const r = await api.post(`/${kind}/import/xml`, txt, { headers: { 'Content-Type': 'application/xml' } })
      const d = r.data
      const extra = d.ignoradas > 0 ? ` · ${d.ignoradas} ignorada${d.ignoradas !== 1 ? 's' : ''} (nº inexistente)` : ''
      setXmlResult({ ok: true, msg: `${d.message} ${d.atualizadas} atualizada${d.atualizadas !== 1 ? 's' : ''}${extra}.` })
      await loadStats()
      if (kind === 'resultados') { loadAnalise(); loadValidacao() }
    } catch (e: any) {
      const data = e.response?.data
      setXmlResult({ ok: false, msg: data?.message ?? 'Erro ao importar XML.', errors: data?.errors })
    } finally {
      setXmlBusy(false)
    ...
    <div className="staff-portal staff-portal--tecnico">
      ...
      <section className="staff-hero">
        ...
        <div className="staff-kpis tec-kpis">
          ...
          <button className={`tec-kpi${(stats?.reqsPendentes ?? 0) > 0 ? ' tec-kpi--action' : ''}`} onClick={() => setTab('analise')}>
            ...
            <div className="tec-kpi-val">{loading ? '—' : (stats?.reqsPendentes ?? 0)}</div>
            <div className="tec-kpi-lbl">para analisar</div>
          </button>

          <button className={`tec-kpi${(stats?.reqsDisponiveis ?? 0) > 0 ? ' tec-kpi--ready' : ''}`} onClick={() => setTab('validacao')}>
            <div className="tec-kpi-icon tec-kpi-icon--ready">
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" /></svg>
            </div>
            <div className="tec-kpi-val">{loading ? '—' : (stats?.reqsDisponiveis ?? 0)}</div>
      ...
      <nav className="staff-tabs tec-tabs">
        {([
          ...
          { id: 'analise',   label: 'Análise',           badge: stats?.reqsPendentes ?? 0 },
          { id: 'validacao', label: 'Validação Técnica', badge: stats?.reqsDisponiveis ?? 0 },
        ] as const).map(t => (
      ...
      <main className="staff-content">
        ...
        {/* ── intercâmbio XML/XSD ── */}
        <div className="tec-xml-bar">
          <span className="tec-xml-bar-lbl">Intercâmbio XML</span>
          <div className="tec-xml-grp">
            <span className="tec-xml-grp-lbl">requisições</span>
            <button className="staff-btn-ghost" onClick={() => exportXML('requisicoes')} disabled={xmlBusy}>↧ exportar</button>
            <button className="staff-btn-ghost" onClick={() => reqFileRef.current?.click()} disabled={xmlBusy}>↥ importar</button>
          </div>
          <div className="tec-xml-grp">
            <span className="tec-xml-grp-lbl">resultados</span>
            <button className="staff-btn-ghost" onClick={() => exportXML('resultados')} disabled={xmlBusy}>↧ exportar</button>
            <button className="staff-btn-ghost" onClick={() => resFileRef.current?.click()} disabled={xmlBusy}>↥ importar</button>
          </div>
          <input ref={reqFileRef} type="file" accept=".xml,application/xml,text/xml" style={{ display: 'none' }}
            onChange={e => { const f = e.target.files?.[0]; if (f) importXML('requisicoes', f) }} />
          <input ref={resFileRef} type="file" accept=".xml,application/xml,text/xml" style={{ display: 'none' }}
            onChange={e => { const f = e.target.files?.[0]; if (f) importXML('resultados', f) }} />
        </div>

        {xmlResult && (
          <div className={`tec-xml-banner ${xmlResult.ok ? 'tec-xml-banner--ok' : 'tec-xml-banner--err'}`}>
            <div className="tec-xml-banner-head">
              <span>{xmlResult.ok ? '✓' : '✕'} {xmlResult.msg}</span>
              <button className="tec-xml-close" onClick={() => setXmlResult(null)}>×</button>
            </div>
            {xmlResult.errors && xmlResult.errors.length > 0 && (
              <ul className="tec-xml-errs">
                {xmlResult.errors.map((er, i) => (
                  <li key={i} className="tec-xml-err-line">
                    {er.line != null && <span className="tec-xml-err-no">linha {er.line}</span>}
                    {er.message}
                  </li>
                ))}
              </ul>
                      ...
                      <div className="tec-req-form-inputs">
                        <div className="tec-ff tec-ff--valor">
                          ...
                          <input className={`tec-input tec-input--sm${isCrit ? ' tec-input--crit' : ''}`}
                            type="text" inputMode="decimal" placeholder="ex: 5.2"
                            value={f.valor}
